...
http_get() {
  ...
  if command -v curl >/dev/null 2>&1; then
    if [ -n "$output" ]; then
      curl --fail --location --progress-bar -H 'User-Agent: biliTickerBuy-installer' -o "$output" "$url"
    else
      curl --fail --location --silent --show-error -H 'User-Agent: biliTickerBuy-installer' "$url"
    fi
  elif command -v wget >/dev/null 2>&1; then
    if [ -n "$output" ]; then
      wget -O "$output" --header='User-Agent: biliTickerBuy-installer' "$url"
    else
      wget -qO- --header='User-Agent: biliTickerBuy-installer' "$url"
    fi
  else
